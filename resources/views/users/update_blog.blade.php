...
@section('content')
    <div class="layout-create">
        <div class="dashboard">
            ...
            <p>{{ __('auth.title_update_blog') }}</p>
        </div>
        <form action="{{ route('put-update-blog', ['id' => $blog->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method("PUT")
            <h3>{{ __('auth.title_update_blog') }}</h3>
            <div class="form-item">
            ...
            <div class="form-item">
                <label for="image">{{ __('auth.btn_change_img') }}<span>*</span></label>
                @error ('image')
                    <small>{{ $message }}</small>
                @enderror
                <div class="item-input btn-primary" id="btn-upload-img">{{ __('auth.btn_change_img') }}</div>
                <input type='file' name="image" id="image" hidden>
            </div>
            <div class="img-preview">
                <img src="{{ Vite::asset('storage/app/public/images/' . $blog->image) }}" alt="">
            </div>
            <div class="form-item">
            ...
            <button type='submit'>{{ __('auth.btn_update_blog') }}</button>
        </form>
    </div>
@endsection
